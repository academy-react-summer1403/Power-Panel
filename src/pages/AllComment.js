...
import { ManageComment } from "../core/services/api/comment";
...
const AllComments = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const getAllComments = await ManageComment(1, 10000);
        const getComments = await ManageComment(currentPage, 10);
        const getAcceptedComments = await ManageComment(
          currentPage,
          10000,
          undefined,
          undefined,
        ...
        const getNotAcceptedComments = await ManageComment(
          currentPage,
          10000,
          undefined,
          undefined,
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const getComments = await ManageComment(
          currentPage,
          10,
          undefined,
          undefined,
