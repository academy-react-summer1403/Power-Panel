...
// ** Table Columns
import { AssistanceWorkColumns } from "../@core/components/AssistanceWork/cloumns";

import { GetAssistanceWorkApi } from "../core/services/api/AssistanceWorkApi";
...
const AssistanceWorkList = () => {
  ...
  useEffect(() => {
    const fetchCourseAssistance = async () => {
      try {
        const res = await GetAssistanceWorkApi();
  ...
  const handleFilter = (val) => {
    textTimeOut(() => {
      ...
      const filtered = CourseAssistance.filter((item) =>
        item.worktitle.toLowerCase().includes(val.toLowerCase()) 
      );
