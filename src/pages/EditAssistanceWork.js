...
import {
  EditAssistanceWorkApi,
  GetAssistanceWorkById,
} from "../core/services/api/AssistanceWorkApi";
...
const EditAssistanceWork = () => {
  ...
  const OnSubmit = async (value) => {
    ...
    try {
      const res = await EditAssistanceWorkApi(formattedData);
      if (res.success === true) {
  ...
  useEffect(() => {
    const fetchGetAssistanceById = async () => {
      try {
        const res = await GetAssistanceWorkById(param.id);
        if (res) {
          setDetail(res);
          reset({
            worktitle: res.worktitle,
            workDescribe: res.workDescribe,
            assistanceId: res.id,
            id: res.workId,
          });
        }
...
export default EditAssistanceWork;
